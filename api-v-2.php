...
foreach ($categories_to_export as $category) {
    ...
    // Генерируем имя файла с учетом структуры категорий
    $safe_alias = preg_replace('/[^a-zA-Z0-9_-]/', '_', $category['alias'] ?? 'noalias');
    $filename = $export_dir . '/category_' . $category_id . '_' . $safe_alias . '.json';
    ...
    if (file_put_contents($filename, json_encode($json_structure, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT))) {
        ...
        echo " (Category: {$category['title']}, Level: {$category['level']})\n";
    } else {
...
// Выводим статистику
echo "\nExport completed!\n";
echo "Total categories exported: " . count($categories_to_export) . "\n";
?>